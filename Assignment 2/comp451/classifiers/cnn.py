...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        W1 = np.random.normal(mu, sigma, (num_filters, C, filter_size, filter_size))
        b1 = np.zeros((num_filters,))

        W2 = np.random.normal(mu, sigma, (num_filters*int(H/2)*int(W/2), hidden_dim))
        b2 = np.zeros((hidden_dim,))

        W3 = np.random.normal(mu, sigma, (hidden_dim, num_classes))
    ...
    def loss(self, X, y=None):
        ...
        h1_out, h1_cashe = conv_relu_pool_forward(X, W1, b1, conv_param, pool_param)
        h2_out, h2_cashe = affine_lrelu_forward(h1_out, W2, b2, {'alpha':self.alpha})
        scores, h3_cashe = affine_forward(h2_out, W3, b3)
        ...
        dx_1, dw_1, db_1 = conv_relu_pool_backward(dx_2, h1_cashe)
