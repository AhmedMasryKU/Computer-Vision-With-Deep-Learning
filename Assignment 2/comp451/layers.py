...
def affine_backward(dout, cache):
    ...
    db = np.matmul(dout.T, np.ones((x.shape[0],)))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H_F = int(1 + (H + 2 * pad - HH) / stride)
    W_F = int(1 + (W + 2 * pad - WW) / stride)

    out = np.zeros(shape = (N, F, H_F, W_F))
    ...
    for i in range(N):
      for j in range(F):
        for k in range(H_F):
          for l in range(W_F):
            out[i, j, k, l] =  np.sum(
              padded_x[i, :, k*stride:k*stride+HH, l*stride:l*stride+WW]*w[j, :]) + b[j]
...
def max_pool_forward_naive(x, pool_param):
    ...
    N, C, H, W = x.shape
    H_F = int(1 + (H - pool_height) / stride)
    W_F = int(1 + (W - pool_width) / stride)

    out = np.zeros(shape = (N, C, H_F, W_F))
      ...
      for j in range(C):
        for k in range(H_F):
          for l in range(W_F):
            ...
            dx[i, j, k*stride + r, l*stride + p] = dout[i, j, k, l]
