...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            ...
            tmp_aff_out, tmp_aff_cache = temporal_affine_forward(rnn_out, W_vocab, b_vocab)
            loss, dout = temporal_softmax_loss(tmp_aff_out, captions_out, mask)

        dx, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dout, tmp_aff_cache)
    ...
    def clip_grad_norm(self, grads, gclip):
        ...
        clipped_grads = {}
        for k in grads:
            clipped_grads[k] = grads[k] * gclip / np.linalg.norm(grads[k])
    ...
    def sample_greedily(self, features, max_length=30):
        ...
        for i in range(max_length):
            word_out, _ = word_embedding_forward(curr_word, W_embed)
            next_h, _  = rnn_step_forward(word_out, next_h, Wx, Wh, b)
            word_scores, _ = affine_forward(next_h, W_vocab, b_vocab)
            curr_word = np.argmax(word_scores, 1)
            #print(curr_word.shape, captions[:, i].shape, captions.shape)
    ...
    def sample_randomly(self, features, max_length=30):
        ...
        for i in range(max_length):
            ...
            next_h, _  = rnn_step_forward(word_out, next_h, Wx, Wh, b)
            word_scores, _ = affine_forward(next_h, W_vocab, b_vocab)
            curr_word = np.zeros((word_scores.shape[0],), dtype = int)
            #print(word_scores.shape)
            for j in range(curr_word.shape[0]):  
                e_x = np.exp(word_scores[j] - np.max(word_scores[j]))
                probs =  e_x / e_x.sum(axis=0)
                curr_word[j] = np.random.choice(np.arange(word_scores.shape[1]), 1, p = probs)
            #print(curr_word.shape, captions[:, i].shape, captions.shape)
