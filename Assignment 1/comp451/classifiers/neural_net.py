...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        h1_r = np.maximum(0, h1)
        h2 = np.matmul(h1_r, W2) + b2
        h2_r = np.maximum(0, h2)
        scores = np.matmul(h2_r, W3) + b3
        ...
        max_scores = max_scores.reshape((len(max_scores), 1))
        shifted_scores = scores - max_scores
        losses = -np.log((np.exp(shifted_scores) / np.sum(np.exp(shifted_scores), axis=1, keepdims=True))[np.arange(N), y])
        loss = np.sum(losses)
        loss /= N
        ...
        dSoftmax = np.exp(shifted_scores) / np.sum(np.exp(shifted_scores), axis=1, keepdims=True)
        dSoftmax[np.arange(N), y] -= 1
        dSoftmax /= N

        dW3 = np.matmul(np.transpose(h2_r), dSoftmax)
        db3 = np.sum(dSoftmax, axis = 0)
        dW3 += reg * 2 * W3

        dW2 = np.matmul(dSoftmax,np.transpose(W3))
        dh2 = dW2 * (h2 > 0)
        dW2 = np.matmul(np.transpose(h1_r), dh2)
        db2 = np.sum(dh2, axis=0)
        dW2 += reg * 2 * W2

        #print("dSoftmax: ", dSoftmax.shape, " W2: ", W2.shape, " dW2: ", dW2.shape, "X: ", X.shape, " W1: ", W1.shape, " W3: ", W3.shape)
        dW1 = np.matmul(dSoftmax, np.transpose(W3))
        dW1 = np.matmul(dW1 * (h2>0), np.transpose(W2))
        dh1 = dW1 * (h1 > 0)
        dW1 = np.matmul(np.transpose(X), dh1)
        dW1 += reg * 2 * W1
    ...
    def predict(self, X):
        ...
        h1 = np.maximum(0, np.matmul(X, self.params['W1']) + self.params['b1'])
        h2 = np.maximum(0, np.matmul(h1, self.params['W2']) + self.params['b2'])
        h3 = np.matmul(h2, self.params['W3']) + self.params['b3']
        y_pred = np.argmax(h3, axis = 1)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
