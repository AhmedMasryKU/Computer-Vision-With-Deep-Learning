...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                import math
                dists[i,j] = math.sqrt(np.sum(np.square(np.subtract(self.X_train[j], X[i]))))
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        dists = np.sqrt(np.sum(np.square(X), axis=1).reshape(num_test, 1) + np.sum(np.square(self.X_train) , axis=1) - 2 * np.matmul(X, np.transpose(self.X_train)))
    ...
    def compute_L1_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dists[i,j] = np.sum(np.absolute(np.subtract(self.X_train[j], X[i])))
    ...
    def compute_L1_distances_no_loops(self, X):
        ...
        # This was commented because it breaks the memory.

        #dists = np.sum(np.absolute(X[:, None] - self.X_train), 2)
